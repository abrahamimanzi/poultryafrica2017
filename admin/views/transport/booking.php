...
<?php include 'transport-content_header'.PL;?>
<?php include 'transport-content_navbar'.PL;?>
	  ...
	  <div class="row">
          <div class="col-sm-12 col-md-12">
              ...
              <div>
                ...
                <?php 
                    if(@empty($_SESSION['transport_sort'])){
                        $_SESSION['transport_sort'] = "`ID` DESC";  
                         Redirect::to("http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]");
                    }
                    if(Input::checkInput('sort-id','post','0')){
                        $_SESSION['transport_sort'] = $_SESSION['transport_sort'] == "`ID` ASC" ? "`ID` DESC" : "`ID` ASC";
                         Redirect::to("http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]");
                    }
                    if(Input::checkInput('sort-name','post','0')){
                        $_SESSION['transport_sort'] = $_SESSION['transport_sort'] == "`firstname` ASC" ? "`firstname` DESC" : "`firstname` ASC";
                         Redirect::to("http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]");
                    }
                    if(Input::checkInput('sort-email','post','0')){
                        $_SESSION['transport_sort'] = $_SESSION['transport_sort'] == "`email` ASC" ? "`email` DESC" : "`email` ASC";
                         Redirect::to("http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]");
                    }
                    if(Input::checkInput('sort-car','post','0')){
                        $_SESSION['transport_sort'] = $_SESSION['transport_sort'] == "`car` ASC" ? "`car` DESC" : "`car` ASC";
                         Redirect::to("http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]");
                    }
                    if(Input::checkInput('sort-room','post','0')){
                        $_SESSION['transport_sort'] = $_SESSION['transport_sort'] == "`room` ASC" ? "`room` DESC" : "`room` ASC";
                         Redirect::to("http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]");
                    }
                    if(Input::checkInput('sort-added_date','post','0')){
                        $_SESSION['transport_sort'] = $_SESSION['transport_sort'] == "`added_date` ASC" ? "`added_date` DESC" : "`added_date` ASC";
                         Redirect::to("http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]");
                    }
                    if(Input::checkInput('sort-payment_state','post','0')){
                        $_SESSION['transport_sort'] = $_SESSION['transport_sort'] == "`payment_state` ASC" ? "`payment_state` DESC" : "`payment_state` ASC";
                         Redirect::to("http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]");
                    }
                    if(Input::checkInput('sort-state','post','0')){
                        $_SESSION['transport_sort'] = $_SESSION['transport_sort'] == "`state` ASC" ? "`state` DESC" : "`state` ASC";
                         Redirect::to("http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]");
                    }
                ...
                <div class="box">
                    ...
                    <?php
                        ...
                        $accommodationTable = new Transport();
                        ...
                        $accommodationTable->selectQuery("SELECT * FROM `transport` WHERE `state`!=? AND `event_ID`=? $search_sql ",$sql_val_array);
                        $accommodationApprovalNumber = $accommodationTable ->count();
                    ?>
                      <h3 class="box-title">Transport list <span style='color:#01aef0'><?=$accommodationApprovalNumber?></span></h3>
                          ...
                          <thead>
                            <form method="post">
                              <tr>
                                ...
                                <th><button type="submit" name="sort-pass_type">Car</button></th>
                          ...
                          <tbody>
                            ...
                            $seconds = \Config::get('time/seconds');
                              $accommodationTable = new Transport();
								...
								$accommodationTable->selectQuery("SELECT* FROM `transport` WHERE `state`!=? AND `event_ID`=? $search_sql ORDER BY {$_SESSION['transport_sort']}",$sql_val_array);
                            ...
                            if(Input::checkInput('request','post',1)){
                                ...
                                $accommodationTable->selectQuery("SELECT * FROM `transport` WHERE `added_date` >= '$from_date' AND `added_date` <= '$to_date'");
                              ...
                              $accommodationTable = new Transport();
                              $accommodationTable->selectQuery("SELECT * FROM `transport` WHERE `state`!=? AND `event_ID`=? $search_sql ORDER BY {$_SESSION['transport_sort']} LIMIT {$offsetNumber},{$rowsLimit}",$sql_val_array);
