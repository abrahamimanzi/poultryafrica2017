...
if(Input::checkInput('id','get','1')){
    ...
    $participantTable->selectQuery("SELECT `code`,`payment_rn`,`payment_state`,`payment_date`,`firstname`,`lastname`,`othername`,`email`,`unit_price`,`amount`,`discount`,`currency`,`category`,`pass_type`,`state`,`receipt_number`,`transaction_number`,`card_issue`,`residence_country`,`residence_city`,`telephone` FROM `events_participant` WHERE `ID`=?  ORDER BY `ID` DESC LIMIT 1",array($participant_ID));
